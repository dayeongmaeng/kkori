...
- 캐시 비우기: `AsyncStorage.clear()` 제거 완료. 앱 전체 스토리지 삭제 대신 캐시 키만 선별 삭제하며 `try/catch` 처리 완료.
- 정책/약관/업데이트 소식: Notion 공개 링크로 연결 완료.
- 알림 섹션: 전체 주석 처리하고, 알림 권한은 권한 섹션으로 이동 완료.
- 꼬리 흔들게 하기: 이스터에그로 유지.
- 꼬리 응원하기: 토스 후원 링크 기반 후원 UI 추가 완료.
